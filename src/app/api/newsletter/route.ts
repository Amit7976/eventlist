...
export async function POST(request:NextRequest) {
  try {
    ...
    if (newsletterExists) {
      return NextResponse.json({ success: false, msg: "Email is already subscribed to the newsletter" }, { status: 409 });
    }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!report) {
      return NextResponse.json({ success: false, msg: "Subscriber not found" }, { status: 404 });
    }
